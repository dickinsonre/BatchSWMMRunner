...
- [SWMM's Data Model](#swmms-data-model)
- [Files Used by SWMM](#files-used-by-swmm)
- [Units of Measurement](#units-of-measurement)
- [Running the SWMM Engine](#running-the-swmm-engine)
- [Examining Object Properties](#examining-object-properties)
- [Retrieving Simulation Results](#retrieving-simulation-results)
- [Controlling Simulation Conditions](#controlling-simulation-conditions)
- [Language Bindings](#language-bindings)
  - [C/C++ Example](#cc++-example)
  - [Pascal Example](#pascal-example)
  - [Python Example](#python-example)
- [Appendix A. SWMM 5's API Functions](#appendix-a-swmm-5s-api-functions)
...
- [Appendix B. SWMM 5's Input File Format](#appendix-b-swmm-5s-input-file-format)
  - [[TITLE]](#title)
  - [[OPTIONS]](#options)
  - [[REPORT]](#report)
  - [[FILES]](#files)
  ...
  - [[CONTROLS]](#controls)
    ...
    - [Modulated Controls](#modulated-controls)
  ...
  - [[PATTERNS]](#patterns)
- [Appendix C. SWMM 5's Output File Format](#appendix-c-swmm-5s-output-file-format)
  - [Output File - Opening Records](#output-file---opening-records)
  - [Output File - Object ID Names](#output-file---object-id-names)
  - [Output File - Object Properties](#output-file---object-properties)
  - [Output File - Reporting Variables](#output-file---reporting-variables)
  - [Output File - Reporting Interval](#output-file---reporting-interval)
  - [Output File - Computed Results](#output-file---computed-results)
  - [Output File - Closing Records](#output-file---closing-records)
...
- **Appendix A** - describes each API function in detail

- **Appendix B** - describes SWMM's input file format

- **Appendix C -** describes the format of SWMM's output results file
...
| char errMsg[241]; int err; double elapsedTime; swmm_open("InpFile.inp", "RptFile.rpt", ""); swmm_start(1); do { swmm_step(&elapsedTime); } while (elapsedTime > 0) swmm_end(); swmm_report(); err = swmm_getError(errMsg, 240); swmm_close(); if (err > 0) printf("\nRun was unsuccessful. %s\n", errMsg); else printf("\nRun was successful.\n"); |
| --- |
...
| char name{21]; swmm_open("InpFile.inp", "RptFile.rpt", ""); for (int i = 0; i < swmm_getCount(swmm_LINK); i++) { if (swmm_getValue(swmm_LINKTYPE, i) == swmm_CONDUIT) { swmm_getName(swmm_LINK, i, name, 20); printf("\n%20s %10.2f %10.4f %10.2f", name, swmm_getValue(swmm_LENGTH, i), swmm_getValue(swmm_SLOPE, i), swmm_getValue(swmm_FULLDEPTH, i)); } } swmm_close(); |
| --- |
...
| vector<double> linkTimeSeries; double elapsedTime; swmm_open("InpFile.inp", "RptFile.rpt", ""); int linkIndex = swmm_getIndex(swmm_LINK, "TheLink"); swmm_start(1); do { swmm_step(&elapsedTime); } while (elapsedTime > 0) swmm_end(); for (int i = 1; i <= swmm_getValue(swmm_TOTALSTEPS, 0); i++) linkTimeSeries.push_back( swmm_getSavedValue(swmm_LINK_FLOW, linkIndex, i); swmm_close(); |
| --- |
...
| int c1, c2, o1; double elapsedTime, fullDepth; swmm_open("InpFile.inp", "RptFile.rpt", ""); c1 = swmm_getIndex(swmm_LINK, "C1"); c2 = swmm_getIndex(swmm_LINK, "C2"); o1 = swmm_getIndex(swmm_LINK, "O1"); fullDepth = swmm_getValue(swmm_LINK_FULLDEPTH, c2); swmm_start(1); swmm_setValue(swmm_LINK_SETTING, o1, 0.0); do { swmm_step(&elapsedTime); if (swmm_getValue(swmm_LINK_FLOW, c1) > 5 && swmm_getValue(swmm_LINK_DEPTH, c2)/fullDepth < 0.5) { swmm_setValue(swmm_LINK_SETTING, o1, 1.0); } else swmm_setValue(swmm_LINK_SETTING, o1, 0.0); } while (elapsedTime > 0); swmm_end(); swmm_report(); swmm_close(); |
| --- |
...
To accommodate the Python language, the following API functions have their original definitions (as documented in Appendix A) modified:


| Original C/C++ Declaration | Python Declaration | Python Return Value |
| --- | --- | --- |
...
| swmm_getMassBalanceErr( float *runoff, float *flow, float *qual) | getMassBalErr() | runoff , flow , and quality continuity errors as a tuple |
...
| swmm_decodeDate(double date int *year, int *month, int *day, int *hour, int *minute, int *second, int *dayOfWeek); | decodeDate(date) | a Python DateTime value |


The following topics illustrate how a simple program that runs a SWMM model from the command line would look in each of these three languages:
...
The code below uses C/C++ to run a SWMM model from the command line with the names of the input and report file as arguments.


| #include "swmm5.h" #include <stdio.h> void run_swmm(char* inpFile, char* rptFile) { double elapsedTime = 0; float runoffErr, routeErr, qualErr; char errMsg[241]; swmm_open(inpFile, rptFile, ""); swmm_start(1); do { swmm_step(&elapsedTime); } while elapsedTime > 0; swmm_end(); swmm_report(); swmm_getMassBalErr(&runoffErr, &routeErr, &qualErr); if (swmm_getError(errMsg, 240) > 0) printf("\n%s", errMsg); else { printf("Runoff Error: %.4f\n", runoffErr); printf("Routing Error: %.4f", routeErr); } swmm_close(); } int main(int argc, char* argv[]) { run_swmm(argv[1], argv[2]); } |
| --- |
...
The code below uses Pascal to run a SWMM model from the command line with the names of the input and report file as arguments.


| program RunSWMM; uses SysUtils, swmm5; procedure Run_Swmm(InpFile: AnsiString; RptFile: AnsiString); var ElapsedTime : Double; RunoffErr, RouteErr, QualErr : Single; ErrMsg: array[0..240] of AnsiChar; begin swmm_open(PAnsiChar(InpFile), PAnsiChar(RptFile), ''); ElapsedTime := 0; swmm_start(1); repeat swmm_step(ElapsedTime); until ElapsedTime = 0; swmm_end(); swmm_report(); swmm_getMassBalErr(RunoffErr, RouteErr, QualErr); if swmm_getError(ErrMsg, 240) > 0 then Writeln(ErrMsg) else begin Writeln(Format('Runoff Error: %.4f', [RunoffErr])); Writeln(Format('Routing Error: %.4f', [RouteErr])); end; swmm_close(); end; begin Run_Swmm(ParamStr(1), ParamStr(2)); end. |
| --- |
...
The code below uses Python to run a SWMM model from the command line with the names of the input and report file as arguments.


| import sys import swmm5 def run_swmm(inpFile, rptFile): swmm5.open(inpFile, rptFile) swmm5.start(1) while True: if swmm5.step() == 0: break swmm5.end() swmm5.report() mass_bal = swmm5.getMassBalErr() errmsg = swmm5.getError() if len(errmsg) > 0: print(errmsg) else: print('Runoff Error: {0:8.4f}'.format(mass_bal[0])) print('Routing Error: {0:8.4f}'.format(mass_bal[1])) swmm5.close() if __name__ == '__main__': run_swmm(sys.argv[1], sys.argv[2]) |
| --- |


## Appendix A. SWMM 5's API Functions
...
```
If f3 is an empty string then results will be written to the report file following the instructions contained in the [REPORT] section of the input file.
```
...
```
If f3 is an empty string then a temporary file will be used to store results saved at each reporting time step. It will be  deleted when the swmm_close function is called.
```
...
| saveFlag | a flag which should be set to 1 if results are to be saved to the binary output file or to 0 if they are not. |
| --- | --- |
...
| elapsedTime | returns the total elapsed time of the simulation (in decimal days) after the current time step has been processed |
| --- | --- |
...
```
When the end of the simulation period is reached the value of elapsedTime will be set to 0.
```


```
After swmm_start is called, this function should be called in a loop until elapsedTime becomes 0 (or an error condition occurs). For example:
```


| double elapsedTime; swmm_start(1); do { swmm_step(&elapsedTime); } while (elapsedTime > 0.0); swmm_end(); |
| --- |
...
| strideStep | the number of seconds to advance the simulation |
| --- | --- |
| elapsedTime | returns the total elapsed time of the simulation (in decimal days) after the strideStep time step has been taken |
...
Retrieves the mass balance continuity errors after a SWMM simulation has ended.

**Arguments**


| runoff | percent continuity error for runoff flow. |
| --- | --- |
| flow | percent continuity error for flow routing. |
| qual | percent continuity error for water quality routing. |
...
```
This function should be called after a simulation has ended with a call to swmm_end.
```
...
```
The subcatchments, nodes, and links whose results are to be reported must be identified in the [REPORT] section of the SWMM input file.
```


```
Instead of using swmm_report an application can generate customized reports by writing code that makes use of the swmm_getValue, swmm_getSavedValue, and swmm_writeLine functions.
```
...
| errMsg | character array to hold error message text. |
| --- | --- |
| msgLen | maximum number of characters that errMsg can hold. |
...
```
The errMsg array must be dimensioned to hold at least msgLen + 1 characters by the calling program. A size of 240 characters should be sufficient.
```
...
| objType | type of SWMM object. |
| --- | --- |
| index | the position (from 0) of the object in the list of like objects. |
| name | a character array that holds the retrieved ID name. |
| size | the maximum number of characters that name can store. |
...
- The *name* string must be dimensioned to *size+1* or greater by the calling program.

- If the specified object does not exist then *name* is returned as an empty string.
...
| objType | type of SWMM object. |
| --- | --- |
| name | the ID name of a specific object type. |
...
| property | a property code associated with an object. |
| --- | --- |
| index | the position (from 0) of the object in the list of like objects. |
...
| Nodes | swmm_NODE_TYPE | swmm_JUNCTION = 0, swmm_OUTFALL = 1, swmm_STORAGE = 2, swmm_DIVIDER = 3 |
...
| swmm_NODE_VOLUME | current computed stored volume (ft 3 or m 3 ) |  |
| swmm_NODE_LATFLOW | current lateral inflow (flow units) |  |
| swmm_NODE_INFLOW | current total inflow (flow units) |  |
| swmm_NODE_OVERFLOW | current overflow (flooding) (flow units) |  |
| swmm_NODE_RPTFLAG | = 1 if results saved to output file, 0 if not |  |
| Links | swmm_LINK_TYPE | swmm_CONDUIT = 0, swmm_PUMP = 1, swmm_ORIFICE = 2, swmm_WEIR = 3, swmm_OUTLET = 4 |
| swmm_LINK_NODE1 | index of upstream node |  |
| swmm_LINK_NODE2 | index of downstream node |  |
...
| swmm_FLOWUNITS | code number of flow units used by a project: swmm_CFS = 0 (cubic feet / second) swmm_GPM = 1 (gallons / minute) swmm_MGD = 2 (million gallons / day) swmm_CMS = 3 (cubic meters / second) swmm_LPS = 4 (liters / second) swmm_MLD = 5 (million liters / day) |  |
...
| --- | --- |
| index | the position (from 0) of the object in the list of like objects. |
| period | a reporting time period index (starting from 1) |
...
```
This function allows one to retrieve computed values saved to SWMM's output results file after a simulation has been completed only if the swmm_start function was called with an argument equal to 1.
```
...
| swmm_NODE_VOLUME | stored volume (ft 3 or m 3 ) |  |
...
| swmm_LINK_TOPWIDTH | water surface width (ft or m) |  |
...
```
Only objects that were marked for reporting in the [REPORT] section of the SWMM input file will have their computed results saved to SWMM's output file. One can see if this is the case by using the swmm_getValue function to check an object's RPTFLAG property. If the object was not marked for reporting then swmm_getSavedValue will return 0.
```
...
The table below lists the property codes and the quantities they represent.
...
| double elapsedTime, routeStep; swmm_start(1); do { swmm_step(&elapsedTime); routeStep = swmm_getValue(swmm_MAXROUTESTEP, 0); swmm_setValue(swmm_ROUTESTEP, 0, routeStep); } while (elapsedTime > 0.0); swmm_end(); |
| --- |
...
```
line        a character string containing the text to be written.
```
...
| date | a DateTime value (as decimal days since Dec 30, 1899) to be decoded |
| --- | --- |
| year | the year encoded in the date |
| month | the month of the year encoded in the date ( 1 to 12) |
| day | the day of the month encoded in the date (1 to 31) |
| hour | the hour encoded in the time portion of the date (0 to 23) |
| minute | the minutes encoded in the time portion of the date (0 to 59) |
| second | the seconds encoded in the time portion of the date (0 to 59) |
| dayOfWeek | the day of the week encoded in the date (Sunday = 1) |
...
```
The date argument is an input parameter while the others are outputs.
```


## Appendix B. SWMM 5's Input File Format
...
| Vratio | v oid ratio (volume of void space relative to the volume of solids in the pavement for continuous systems or for the fill material used in modular systems). Note that porosity = void ratio / (1 + void ratio). |
| --- | --- |
...
| Thick | t hickness of the soil layer (inches or mm). |
| --- | --- |
...
The equation used to compute flow rate out of the underdrain per unit area of the LID (in in/hr or mm/hr) is *q = C(h-H**d**)**n*  where *q* is outflow, *h* is height of stored water (inches or mm) and *H**d* is the drain offset height. Note that the units of *C* depend on the unit system being used as well as the value assigned to *n*.
...
| Apond | area subjected to surface ponding once water depth exceeds Ymax + Ysur (ft 2 or m 2 ) |
| --- | --- |
...
| Apond | area subjected to surface ponding once water depth exceeds Ymax + Ysur (ft 2 or m 2 ) (default is 0). |
| --- | --- |
...
| Acurve | name of a curve in the [ CURVES ] section that relates surface area (ft 2 or m 2 ) to depth (ft or m) for TABULAR geometry. |
| --- | --- |
...
| Shape | L | W | Z |
| --- | --- | --- | --- |
| CYLINDRICAL | major axis length | minor axis width | not used |
| CONICAL | major axis length of base | minor axis width of base | side slope (run/rise) |
| PARABOLOID | major axis length at full height | minor axis width at full height | full height |
...
```
Keywords are shown in boldface and ruleID is an ID label assigned to the rule, condition_n is a Condition Clause, action_n is an Action Clause, and value is a priority value (e.g., a number from 1 to 5). The formats used for Condition and Action clauses are discussed below.
```
...
## Appendix C. SWMM 5's Output File Format
...
The byte position of the file where this section begins and the byte position where the next section begins are located in the Closing Records section of the file.


### Output File - Object Properties
...
### Output File - Reporting Variables
...
### Output File - Reporting Interval
